...
import { Upload, Link2, Zap, Plus, X, Calendar, Trash2 } from 'lucide-react'
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            {upcomingEvents.length > 0 && (
              <span className="px-2 py-0.5 rounded-full text-[10px] bg-brand-100 text-brand-700 font-semibold">
        ...
        {upcomingEvents.length === 0 ? (
          ...
          <div className="space-y-2">
            {upcomingEvents.map(ev => {
              ...
              return (
                <div key={ev.id} className={`group flex items-center gap-3 p-3 rounded-xl transition-colors ${today ? 'bg-red-50 border border-red-100' : 'bg-gray-50 hover:bg-gray-100'}`}>
                  <div className={`text-center flex-shrink-0 w-10 ${today ? 'text-red-600' : 'text-gray-600'}`}>
                    <p className="text-[10px] font-medium uppercase">{format(date, 'MMM', { locale: es })}</p>
                    <p className="text-lg font-bold leading-none">{format(date, 'd')}</p>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{ev.title}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className={`inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold ${TYPE_COLORS[ev.type]}`}>
                        {ev.type}
                      </span>
                      {ev.semester && (
                  ...
                  {ev.created_by === user?.id && (
                    <button onClick={() => handleDeleteEvent(ev.id)}
                      className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg bg-red-50 text-red-400 hover:bg-red-100 transition-all flex-shrink-0">

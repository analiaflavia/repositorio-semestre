  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <button onClick={() => setCurrentMonth(m => subMonths(m, 1))} className="p-1 hover:bg-gray-100 rounded-lg transition-colors">
          <ChevronLeft className="w-4 h-4 text-gray-500" />
        </button>
        <span className="text-sm font-semibold text-gray-800 capitalize">
          {format(currentMonth, 'MMMM yyyy', { locale: es })}
        </span>
        <button onClick={() => setCurrentMonth(m => addMonths(m, 1))} className="p-1 hover:bg-gray-100 rounded-lg transition-colors">
          ...
          return (
            <button key={day.toISOString()} onClick={() => setSelectedDay(isSelected ? null : day)}
              className={`relative flex flex-col items-center py-1 rounded-lg transition-colors ${
                isSelected ? 'bg-brand-600' : todayDay ? 'bg-brand-50' : dots.length > 0 ? 'hover:bg-gray-100' : 'hover:bg-gray-50'
              }`}>
              <span className={`text-xs font-medium ${isSelected ? 'text-white' : todayDay ? 'text-brand-700 font-bold' : 'text-gray-700'}`}>
                {format(day, 'd')}
              </span>
              {dots.length > 0 && (
        ...
        <div className="mt-3 border-t border-gray-100 pt-3">
          ...
          {selectedEvents.length === 0 ? (
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => onAdd(selectedDay)} className="flex items-center gap-1 px-2 py-1 bg-brand-600 hover:bg-brand-700 text-white text-[10px] font-medium rounded-lg">
    ...
    return (
      <div className="bg-gray-50 rounded-lg p-2 space-y-1.5">
        ...
        <div className="flex gap-1">
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-1 px-2 py-1 bg-brand-600 text-white text-[10px] rounded">
            <Check className="w-3 h-3" /> {saving ? '...' : 'Guardar'}
          </button>
          <button onClick={() => setEditing(false)} className="flex items-center gap-1 px-2 py-1 bg-gray-100 text-gray-700 text-[10px] rounded">
  ...
  return (
    <div className="group flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 transition-colors">
      ...
      <div className="flex gap-0.5 opacity-0 group-hover:opacity-100">
        <button onClick={() => setEditing(true)} className="p-1 rounded bg-blue-50 text-blue-400 hover:bg-blue-100">
          <Pencil className="w-3 h-3" />
        </button>
        {ev.created_by === user?.id && (
          <button onClick={() => onDelete(ev.id)} className="p-1 rounded bg-red-50 text-red-400 hover:bg-red-100">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => { setNewDate(''); setShowForm(p => !p) }}
                className="flex items-center gap-1 px-2 py-1 bg-brand-600 hover:bg-brand-700 text-white text-[10px] font-medium rounded-lg transition-colors">
                <Plus className="w-3 h-3" /> Agregar
              </button>
            </div>
